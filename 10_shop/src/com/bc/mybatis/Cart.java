...
public class Cart {
	...
	public void addProduct(String p_num, ShopDAO dao) {
		...
		if (vo != null) {//카트에 동일한 제품이 있다. -> 수량 증가 처리
			//1. 수량 증가 처리
			vo.setQuant(vo.getQuant() + 1);
			//2. 카트의total 값 합산(제품 하나 가격 추가) 세일가격
			total += vo.getP_saleprice();
			
		} else {//카트에 동일한 제품 없음 -> 제품을 카트에 등록
	...
	//카트목록(list) 제품의 수량 변경 처리
	public void setQuant(String p_num, int su) {
		ShopVO vo = findProduct(p_num);
		if (vo == null) return;
		
		//1. 카트 합계금액 - 수량변경 전 제품합계금액
		total = total - vo.getTotalprice();
		
		//2. 제품의 수량 변경
		vo.setQuant(su);
		
		//3. 카트 합계금액 + 수량변경 후 제품합계금액
		total = total + vo.getTotalprice();
	}
